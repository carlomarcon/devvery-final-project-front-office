...
export default {
  ...
  computed: {
    numberOfProducts() {
      return this.store.cartData.reduce(
  ...
  <header>
    <nav class="navbar fixed-top bg-body-tertiary flex-nowrap">
      <div class="container">
        ...
        >
          <router-link class="btn d-none d-md-block fw-bold" to="/about-us"
            >CHI SIAMO</router-link
          >
          <a
            href="http://127.0.0.1:8000/admin"
            class="btn ms_btn-yellow d-block"
            >LOGIN</a
          >
          <button
            class="navbar-toggler border-black border-2 d-flex align-content-center gap-1 position-relative"
            type="button"
            ...
            @click="closeModal()"
          >
            <div class="counter" v-if="store.cartData.length != 0">
              <span class="d-inline-block ms_color-dark fw-bold">{{
                numberOfProducts
              }}</span>
            </div>
...
button:hover {
  background-color: rgba($color: rgb(235, 232, 232), $alpha: 1);
}
</style>
